...
///
/// Writes through `SaverPreferences`, which mirrors every value into standard
/// defaults as well as the module's own ByHost store. The options sheet is
/// presented from a sandboxed host where the ByHost write is not guaranteed to
/// land, so without the mirror a choice made in System Settings can simply not
/// be there next time. The Solar System saver hit that first.
